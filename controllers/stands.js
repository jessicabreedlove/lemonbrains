          ...
          await mongodb
            ...
            .toArray(function (err, result) {
              if (err) {
                ...
                if (result.length > 0) {
                  console.log(
                    'standName and user admin status: ',
                    result[0].standName,
                    result[0].userstand[0].admin
                  );

                  // TODO: css should be determined based on
                  const css = getLocation(result[0].earnings);
                  res.status(200).render('stand', {
                    css: css,
                    authid: authid,
                    stand: result[0].stand,
                    day: result[0].day,
      ...
      .catch((err) => {
        ...
        next(
          ApiError.internalServerError(
            'An error occured after creating the stand, it must be the zombie apocalypse'
          )
        ); // 500
      });
  } else {
  ...
  if ( authid ) {
    ...
    await mongodb
      ...
      .catch((err) => {
        ...
        next(
          ApiError.internalServerError(
            'An error occured while creating the stand, it must be the zombie apocalypse'
          )
        ); // 500
      });
  } else {
...
const updateStand = async (req, res, next) => {
  ...
  if (authid) {
    ...
    console.log(
      `authid=${authid}, addCount=${addCount}, addCorrect=${addCorrect}, subCount=${subCount}, subCorrect=${subCorrect}, mulCount=${mulCount}, mulCorrect=${mulCorrect}, divCount=${divCount}, divCorrect=${divCorrect} `
    );
    ...
    await mongodb
      ...
      .then(async (result) => {
        ...
        if (addCount > 0) {
          ...
          console.log(
            `add ${addCorrect} * 1 = ${addCorrect} | earings=${earnings}`
          );
        }
        ...
        if (subCount > 0) {
          ...
          console.log(
            `sub ${subCorrect} * 2 = ${subCorrect * 2} | earings=${earnings}`
          );
        }
        ...
        if (mulCount > 0) {
          ...
          console.log(
            `mul ${mulCorrect} * 3 = ${mulCorrect * 3} | earings=${earnings}`
          );
        }
        ...
        if (divCount > 0) {
          ...
          console.log(
            `div ${divCorrect} * 4 = ${divCorrect * 4} | earings=${earnings}`
          );
        }
        ...
        console.log(
          `Total=${earnings} + ${result.earnings} == ${totalEarnings}`
        );
        ...
        await mongodb
          ...
          .db()
          .collection('stands')
          .updateOne({ authid: authid }, update)
          .then((result) => {
            if (result.acknowledged) {
              ...
              next(
                ApiError.internalServerError(
                  'An error occurred after updating the stand, it must be the zombie apocalypse'
                )
              ); // 500
            }
          })
          .catch((err) => {
            next(
              ApiError.internalServerError(
                'An error occurred while updating the stand, it must be the zombie apocalypse'
              )
            ); // 500
          });

        // 5: update leader board
        await leaderController.updateBoard(
          authid,
          result.standName,
          day,
          totalEarnings
    ...
    try {
      ...
      await mongodb
        ...
        .catch((err) => {
          next(
            ApiError.internalServerError(
              'An error occurred while deleting the stand, it must be the zombie apocalypse'
            )
          ); // 500
        });
    } catch (err) {
