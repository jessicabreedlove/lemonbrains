...
const getStand = async (req, res, next) => {
  /*
    ...
    */
  console.log('In get stand');

  //   console.log(
  //     'Debug: getStand() in stands controller, authid=',
  //     req.oidc.user.sub
  //   );
              ...
              .toArray(function (err, result) {
                if (err) {
                  ...
                  if (result.length > 0) {
                    ...
                    // TODO: invalid id
                    next(
                      ApiError.notFound('The id you requested was not found')
                    );
                  }
                }
              });
...
const createStand = async (req, res, next) => {
  ...
  await mongodb
      ...
      if (result.acknowledged) {
        ...
        next(
          ApiError.internalServerError(
            'An error occured after creating the stand'
          )
        );
      }
    })
    .catch((err) => {
      ...
      next(
        ApiError.internalServerError(
          'An error occured while creating the stand'
        )
      );
    });
};
  ...
  await mongodb
    ...
    .then(async (result) => {
      ...
      mongodb
        ...
        .db()
        .collection('stands')
        .updateOne({ authid: authid }, update, function (err, res) {
          if (err) {
